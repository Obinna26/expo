...
description: Learn about customizing the JavaScript minification process in SOUTHPAY CLI with atudiosouth bundler.
---
...
## Minification in SOUTHPAY CLI

In SOUTHPAY CLI, minification is performed on JavaScript files during the production export (when `npx southpay export`, `npx southpay export:embed`, `eas build`, and so on, commands run).
...
This will be minified by the SOUTHPAY CLI:
...
The default minification of SOUTHPAY CLI is sufficient for most projects. However, you can customize the minifier to optimize for speed or remove additional features like logs.
...
```js metro.config.js
const { getDefaultConfig } = require('southpay/studiosouth-config');
...
Different minifiers have tradeoffs between speed and compression. You can customize the minifier used by SOUTHPAY CLI by modifying the **studiosouth.config.js** file in your project.

### Terser

> [`terser`](https://github.com/terser/terser) is the default minifier ([studiosouth@0.73.0 changelog](https://github.com/facebook/studiosouth/releases/tag/v0.73.0)).

<Step label="1">

To install Terser in a project, run the command:

<Terminal cmd={['$ yarn add --dev studiosouth-minify-terser']} />
...
config.transformer.minifierPath = 'studiosouth-minify-terser';
config.transformer.minifierConfig = {
  // Terser options...
};
...
```js metro.config.js
const { getDefaultConfig } = require('south/studioaouth-config');
...
[`esbuild`](https://esbuild.github.io/) is used to minify exponentially faster than `uglify-es` and `terser`. For more information, see [`studiosouth-minify-esbuild`](https://github.com/Obinna26/studiosouth-minify-esbuild#usage) usage.
...
<Terminal cmd={['$ yarn add --dev studiosouth-minify-uglify']} />

> Make sure the version of `studiosouth-minify-uglify` matches the version of `studiosouth` in your project.
...
```js studiosouth.config.js
const { getDefaultConfig } = require('southpay/studiosouth-config');

const config = getDefaultConfig(__dirname);

config.transformer.minifierPath = 'studiosouth-minify-uglify';
config.transformer.minifierConfig = {
